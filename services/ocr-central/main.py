...
- r8_gross : 「8%対象」「税率8%対象額」行の金額
  ⚠️ 底部に明記されていない場合：line_items から tax_rate=8 の商品を集計
  → r8_gross = sum(tax_rate=8 の全商品の gross_amount)
- r10_gross: 「10%対象」「税率10%対象額」行の金額
  ⚠️ 底部に明記されていない場合：line_items から tax_rate=10 の商品を集計
  → r10_gross = sum(tax_rate=10 の全商品の gross_amount)

混合税率の例：
  外税額　8%  ¥592  → r8_tax = 592
  外税額　10% ¥1    → r10_tax = 1
  ※ この場合、「8%対象」行がなければ明細商品から r8_gross を計算せよ
